...
namespace Estates.Engine
{
    class EstateEngine : IEstateEngine
    {
        ...
        private string ExecuteFindRentsByPriceCommand(string fromPrice, string toPrice)
        {
            decimal resFromPrice = Decimal.Parse(fromPrice);
            decimal resToPrice = Decimal.Parse(toPrice);

            var offers = this.Offers
               .Where(o => o.Type == OfferType.Rent)
               .OrderBy(o => o.Estate.Name);
            var offersRents = offers
               .Where(o => ((IRentOffer) o).PricePerMonth >= resFromPrice && 
                   ((IRentOffer) o).PricePerMonth <= resToPrice)
               .OrderBy(o => ((IRentOffer) o).PricePerMonth);
           return FormatQueryResults(offersRents);
        }
